...
from service_pb2 import PlayerAction, CardType, Tackle, LoggerLevel
...
class BhvBasicTackle:
    ...
    def ExecuteOldVersion(self, agent: IAgent, use_foul: bool):
        ...
        if abs(wm.self.body_direction) < self.body_thr:
            return PlayerAction(tackle=Tackle(power_or_dir=tackle_power, foul=use_foul))
        ...
        if tackle_power < 0.0 and abs(wm.self.body_direction) > 180 - self.body_thr:
            return PlayerAction(tackle=Tackle(power_or_dir=tackle_power))
    ...
    def ExecuteV12(self, agent: IAgent, use_foul: bool):
        ...
        if s_last_execute_time == wm.time():
            if s_result:
                ...
                return PlayerAction(tackle=Tackle(power_or_dir=tackle_dir, foul=use_foul))
